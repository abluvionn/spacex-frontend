...
export default function KnowledgeTestPage() {
  ...
  const [submitTest, { isLoading: submitting }] =
    useDriverSubmitKnowledgeTestMutation();
  ...
  const getOptionEntries = (options: KnowledgeTestQuestion['options']) => {
    if (Array.isArray(options)) {
      ...
      return options.map(
        (value, index) => [letters[index], value] as [string, string],
      );
    }
        ...
        <div className='max-w-4xl mx-auto px-4'>
          <div className='bg-white rounded-lg shadow-lg p-8'>
            ...
            <div className='text-center mb-8'>
              <div
                className={`text-2xl font-bold mb-4 ${results.passed ? 'text-green-600' : 'text-red-600'}`}
              >
